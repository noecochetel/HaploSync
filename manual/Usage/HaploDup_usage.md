...
#### Mandatory
   ...
   * `-g | --gff annotation.gff3`: The gene annotation for the pseudomolecules in GFF3 format. A comma-separated list of annotation files can be provided if the annotation is in multiple files. If not provided, GMAP-based gene analysis is skipped and only dotplots are produced.
...
* `--skip_dotplots_by_chr`: If set, prevents the production of dotplots comparing each chromosome sequence to each other chromosome sequence. A whole genome dotplot is still produced.
