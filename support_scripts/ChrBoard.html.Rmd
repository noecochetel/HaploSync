...
# subset
coords1_all <- tb_1 %>% filter(tID==nucmer_ref , qID==nucmer_query )
coords1 <- coords1_all %>% filter(identity > min_iden , match > min_len )
if (has_self) {
  coords2 <- tb_2 %>% filter(tID==nucmer_ref , qID==nucmer_ref , identity > min_iden , match > min_len )
}
# Use the unfiltered hits for axis bounds so the plot area still reflects
# the true chromosome lengths even when no segment passes the
# identity/length thresholds below (coords1 can then be empty).
refLen <- max(coords1_all$tLen)
queryLen <- max(coords1_all$qLen)
...
# ggplotly() cannot convert an empty geom_segment layer without erroring
# (plotly's to_basic.GeomSegment assumes at least one row), so when no
# alignment passes the identity/length thresholds for this pair, render an
# equivalent placeholder instead of crashing the whole report.
if (nrow(coords1) == 0 || (has_self && nrow(coords2) == 0)) {

  pl0 <- plotly::plot_ly() %>%
    layout(
      title  = nucmer_title,
      xaxis  = list(title = nucmer_ref, range = c(0, refLen)),
      yaxis  = list(title = "Query sequence - Position (bp)", range = c(0, max(refLen, queryLen))),
      annotations = list(text = "No alignments above threshold", showarrow = FALSE,
