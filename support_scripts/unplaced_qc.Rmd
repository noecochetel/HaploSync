...
unpl_len <- if (empty(agp_legacy_unpl)) 0 else max(agp_legacy_unpl$stop)
...
if (!(empty(agp_legacy_unpl))) { p1 <- p1 + geom_rect(data = agp_legacy_unpl, mapping = aes(xmin = start, xmax = stop, ymin = 0,    ymax = 1.5  , fill=group_id ), size = 0.2, color = "black") }
if (!(empty(agp_legacy_unpl))) { p1 <- p1 + geom_text_repel(data = agp_legacy_unpl, mapping = aes(x = (start + stop)/2 , y = 0.75,  label = component), direction = "y" , box.padding = unit(0.1, "line") , size = 3 , ylim = c( 0.51 , 0.99  ) ) }
...
if (!(empty(agp_legacy_unpl))) { p1_noLabels <- p1_noLabels + geom_rect(data = agp_legacy_unpl, mapping = aes(xmin = start, xmax = stop, ymin = 0,    ymax = 1.5 , text = sprintf( "Sequence ID: %s<br>Sequence group: %s" , component , group_id ) , fill=group_id ), size = 0.2, color = "black") }
...
if (!(empty(block_unpl))) { p2_noLabels <- p2_noLabels + geom_rect(data = block_unpl, mapping = aes(xmin = start-.1, xmax = stop+.1, ymin = 0, ymax = 1 , fill=orientation , text = sprintf( "Sequence ID: %s<br>Orientation: %s<br>Marker range: %s-%s" , input_id , orientation , start , stop) ) , size = 0.2, color = "black" ) }
